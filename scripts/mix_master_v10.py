...
SOURCE = ROOT / "outputs/youtube-auto-v10-synced-no-music.mp4"
BGM = ROOT / "audio/bgm-ai-tech-cc0.mp3"
OUTPUT = ROOT / "outputs/youtube-auto-v10-avatar3-bgm-sfx.mp4"
...
def main() -> None:
    ...
    filters = [
        ...
        "[1:a]aresample=48000,volume='if(lt(t,10),0.095,if(between(t,170,216),0.055,if(gt(t,281),0.09,0.075)))':eval=frame,"
        "equalizer=f=2500:t=q:w=1:g=-3:enable='between(t,170,216)',afade=t=in:st=0:d=2[bgm]",
    ...
    for index, cue in enumerate(CUES, start=2):
        ...
        gain = float(cue["gain"])
        filters.append(
            f"[{index}:a]aresample=48000,atrim=0:3,asetpts=PTS-STARTPTS,"
            f"volume={gain},adelay={delay}|{delay}[s{index}]"
        )
        labels.append(f"[s{index}]")
    filters.append("".join(labels) + f"amix=inputs={len(labels)}:duration=first:normalize=0,alimiter=limit=0.90,volume=0.84[aout]")
    command.extend([
